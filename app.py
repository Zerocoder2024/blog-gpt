...
async def get_recent_news(topic):
    ...
    timeout = httpx.Timeout(10.0)  # Устанавливаем тайм-аут для запроса
    async with httpx.AsyncClient(timeout=timeout) as client:
        try:
            ...
            raise HTTPException(status_code=500, detail="Ошибка сети при обращении к NewsAPI")

    articles = response.json().get("articles", [])
    if not articles:
        return "Свежих новостей не найдено."
    recent_news = [article["title"] for article in articles[:2]]  # Ограничим до 2 статей
    return "\n".join(recent_news)
